...
struct QRURLParser {
    static func from(string: String) -> ParserResult? {
        ...
        let partsArr = parts[0].components(separatedBy: ":")
        if parts.count == 1, let address = partsArr.last, CryptoAddressValidator.isValidAddress(address) {
            return ParserResult(
                protocolName: "",
                address: address,
                params: [:]
        ...
        if parts.count == 2, let address = partsArr.last, CryptoAddressValidator.isValidAddress(address) {
            return ParserResult(
                protocolName: "",
                address: address,
