        ...
        Session.send(EtherServiceRequest(batch: BatchFactory().create(request))) { [weak self] result in
            ...
            case .success(let tx):
                ...
                if newTransaction.blockNumber > 0 {
                    ...
                    UserDefaults.standard.removeObject(forKey: newTransaction.id)
                    UserDefaults.standard.removeObject(forKey: newTransaction.id+"1")
                }
            case .failure(let error):
                switch error {
                case .responseError(let error):
                    ...
                    case .resultObjectParseError:
                        if transaction.date > Date().addingTimeInterval(TrustNetwork.deleteMissingInternalSeconds) {
                            ...
                            if transaction.date.addingTimeInterval(5.0) < Date() && (self?.count)!<0 && UserDefaults.standard.data(forKey: transaction.id) == nil{
                                completion(.success((transaction, .deleted)))
                            }
                            //
                        }
